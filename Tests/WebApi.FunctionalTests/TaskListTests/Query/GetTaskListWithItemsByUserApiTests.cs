...
public class GetTaskListWithItemsByUserApiTests : BaseIntegrationTest
{
    ...
    public async Task<string> ArrangeDb()
    {
        ...
        // null task item
        TaskList taskList = new TaskList() { Title = "task1", DueDate = DateOnly.FromDateTime(DateTime.Now), AssignedToId = taskUserId, PriorityId = 1 };
        await _dbContext.TaskLists.AddAsync(taskList);
        await _dbContext.SaveChangesAsync(cancellationToken);

        // 1 task item
        taskList = new TaskList() { Title = "task2", DueDate = DateOnly.FromDateTime(DateTime.Now), PriorityId = 1 };
        ...
        // half completed
        taskList = new TaskList() { Title = "task3", DueDate = DateOnly.FromDateTime(DateTime.Now), AssignedToId = taskUserId, PriorityId = 1 };
        ...
        // all completed
        taskList = new TaskList() { Title = "task4", DueDate = DateOnly.FromDateTime(DateTime.Now), AssignedToId = taskUserId, PriorityId = 1 };
