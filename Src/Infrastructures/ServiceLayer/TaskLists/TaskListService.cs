...
public class TaskListService : ITaskListService
{
    ...
    public async Task<CustomResult<List<CreateTaskListResponse>>> CreateTaskListBulkAsync(CreateTaskListBulkRequest createTaskListBulkRequest,
                                                                                   CancellationToken cancellationToken)
    {
        List<CreateTaskListResponse> createTaskListResponseList = new List<CreateTaskListResponse>();
        ...
        try
        {
            ...
            using var logScope = _logger.BeginScope(new Dictionary<string, object>
            {
                ["ZoneId"] = createTaskListBulkRequest.createTaskListRequest.First().Title,
                ["RequestNumber"] = Interlocked.Increment(ref taskCount)
            });
            ...
            foreach (var request in createTaskListBulkRequest.createTaskListRequest)
            {
                activity?.SetTag("zone.id", request.Title);
            ...
            _logger.LogInformation(
                    "📊 Retrieved forecast for zone {ZoneId} in {Duration:N0}ms with {PeriodCount} periods",
                    createTaskListBulkRequest.createTaskListRequest.First().Title,
                    stopwatch.Elapsed.Milliseconds,
                    createTaskListResponseList?.Count ?? 0);
