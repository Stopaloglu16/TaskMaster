...
- Password-reset mail now comes from `IEmailSender` over SMTP, like the invite mail, and links to this app's own `/resetpassword/{username}/{token}` page — Keycloak's hosted set-password page and its `execute-actions-email` are no longer used. Read it in Papercut from the dashboard. (The realm's own SMTP settings still point at **`papercut:2525`** rather than `papercut:25`, because Keycloak reaches Papercut container-to-container and the `changemakerstudiosus/papercut-smtp` image listens on 2525 internally; the `25` in `AddPapercutSmtp("papercut", 80, 25)` is the *host* port, which is what `EmailSender`'s `SmtpClient("localhost", 25)` uses.)
