...
- **WebApiAuth** — a *separate* controller-based API for identity: register, login, forgot password. It is a **façade over Keycloak**, not a token issuer — see "Authentication" below. It still owns `WebIdentityContext` (whose tables are now vestigial but kept so the migration history stays valid) and the invite-email `IEmailSender` used by `UsersController`.
...
Five things that are load-bearing here:
